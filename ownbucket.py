...
parser = argparse.ArgumentParser(
    description="OwnBucket : Bucket Enumeration Tool")
parser.add_argument(
    "-t", "--target", help="Target to search Buckets for", dest='target', required=True)
parser.add_argument('--gcp', dest='scan_gcp', action='store_true')
parser.add_argument('--azure', dest='scan_azure', action='store_true')
parser.add_argument('--all', dest='scan_all', action='store_true')
args = parser.parse_args()

scan_gcp = args.scan_gcp
scan_azure = args.scan_azure
scan_all = args.scan_all
if scan_gcp or scan_all or scan_azure:
    scan_aws = False
if (scan_gcp or scan_azure) and scan_all:
    banner()
    print(f"{RED} Too many Arguments Provided , Use Either --all , --azure or --gcp{RESET}")
    sys.exit()
...
def check_aws(bucket):
    try:
        ...
        resp = requests.get(
            f"http://{bucket}.s3.amazonaws.com", headers=headers)
...
def check_gcp(bucket):
    try:
        headers = {'User-Agent': f'{ua.random}'}
        resp = requests.get(
            f"https://storage.googleapis.com/{bucket}", headers=headers)
        if resp.status_code == 200:
            print(f"{GREEN}{bucket:15s} : {resp.status_code}{RESET}")
        if resp.status_code == 403:
            print(f"{RED}{bucket:15s} : {resp.status_code}{RESET}")
...
def check_azure(bucket):
    try:
        headers = {'User-Agent': f'{ua.random}'}
        resp = requests.get(
            f"https://{bucket}.blob.core.windows.net/", headers=headers)
        if resp.status_code == 400:
            print(f"{GREEN}{bucket:15s} : Valid Azure Storage Blob{RESET}")
    ...
    if scan_aws:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=no_of_workers) as executor:
            try:
                ...
                sys.exit()
    if scan_azure:
        print(f'[+] Checking Azure Blob Storage for "{target}"\n')
        with concurrent.futures.ThreadPoolExecutor(max_workers=no_of_workers) as executor:
            try:
                executor.map(check_azure, final)
            except KeyboardInterrupt:
                sys.exit()
    if scan_all:
        print(f'[+] Checking AWS S3 Buckets for "{target}"\n')
